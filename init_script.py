"""
Initial script to run after installing package via pip.

Unfortunately dependency package **ifcopenshell** is not distributed 
via PyPi. 
Because of that, error: ModuleNotFoundError: No module named 'ifcopenshell' 
will be raised. This script downloads **ifcopenshell** and moves 
the package directory to the directory with site-packages. 

This script allows user to specify their own optional arguments:
    -s --system - platform system  
    -a --architecture - platform architecture  
    -v --python_version - version of running python  
If not specified, script automatically detects user's system
as well as platform architecture and running python version.
